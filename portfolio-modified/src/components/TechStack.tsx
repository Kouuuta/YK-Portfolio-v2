import React, { useState } from "react";
import { FadeIn } from "./FadeIn";
import { motion, useReducedMotion } from "framer-motion";

const logos: Record<string, React.FC<{ size: number }>> = {
  Django: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Django">
      <rect width="128" height="128" rx="16" fill="#092E20" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <path fill="#2ba977" d="M46.206 0h16.342v74.898c-8.37 1.583-14.533 2.206-21.2 2.206C21.39 77.084 11 68.167 11 51.042c0-16.5 11.02-27.209 28.098-27.209 2.65 0 4.669.209 7.108.834zm.573 38.164c-1.914-.625-3.491-.833-5.51-.833-8.265 0-13.04 5.041-13.04 13.877 0 8.602 4.565 13.352 12.935 13.352 1.808 0 3.28-.102 5.615-.415z" />
        <path fill="#2ba977" d="M89.27 25.844v37.5c0 12.915-.967 19.125-3.806 24.48-2.65 5.147-6.141 8.393-13.355 11.979l-15.163-7.146c7.213-3.352 10.704-6.315 12.934-10.835 2.334-4.623 3.07-9.978 3.07-24.061V25.844zM71.304 0h16.342v16.604H71.303z" />
      </svg>
    </svg>
  ),
  Python: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Python">
      <rect width="128" height="128" rx="16" fill="#1a1a2e" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <g clipPath="url(#py-a)">
          <path fill="url(#py-b)" d="M49.866 0c-4.08.02-7.973.367-11.4.974C28.368 2.757 26.537 6.49 26.537 13.376v9.092h23.856v3.03h-32.81c-6.934 0-13.005 4.168-14.904 12.098-2.191 9.086-2.288 14.758 0 24.246 1.695 7.063 5.745 12.095 12.68 12.095h8.203v-10.9c0-7.875 6.812-14.82 14.903-14.82h23.83c6.633 0 11.928-5.463 11.928-12.123V13.377c0-6.466-5.455-11.323-11.928-12.402-4.1-.682-8.352-.993-12.43-.974M36.964 7.314c2.464 0 4.477 2.046 4.477 4.562 0 2.505-2.013 4.53-4.477 4.53-2.473 0-4.476-2.025-4.476-4.53-.001-2.516 2.003-4.562 4.476-4.562" />
          <path fill="url(#py-c)" d="M77.198 25.498v10.594c0 8.212-6.964 15.125-14.903 15.125h-23.83c-6.527 0-11.928 5.587-11.928 12.124V86.06c0 6.464 5.621 10.268 11.928 12.122 7.551 2.219 14.793 2.621 23.83 0 6.005-1.74 11.927-5.24 11.927-12.122v-9.094H50.394v-3.031h35.758c6.933 0 9.519-4.836 11.93-12.095 2.491-7.473 2.383-14.66 0-24.246-1.714-6.903-4.986-12.097-11.93-12.097zm-13.403 57.53c2.474 0 4.477 2.026 4.477 4.533 0 2.514-2.004 4.56-4.477 4.56-2.464 0-4.476-2.046-4.476-4.56 0-2.507 2.012-4.533 4.476-4.533" />
        </g>
        <defs>
          <linearGradient id="py-b" x1="-1.392" x2="53.633" y1="2.844" y2="49.769" gradientUnits="userSpaceOnUse">
            <stop stopColor="#5a9fd4" />
            <stop offset="1" stopColor="#306998" />
          </linearGradient>
          <linearGradient id="py-c" x1="74.335" x2="54.603" y1="78.937" y2="51.265" gradientUnits="userSpaceOnUse">
            <stop stopColor="#ffd43b" />
            <stop offset="1" stopColor="#ffe873" />
          </linearGradient>
          <clipPath id="py-a"><path fill="#fff" d="M0 0h100v100H0z" /></clipPath>
        </defs>
      </svg>
    </svg>
  ),
  Java: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Java">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <path fill="#5382a1" d="M36.753 77.336s-3.822 2.222 2.72 2.974c7.924.904 11.973.774 20.706-.879 0 0 2.296 1.44 5.502 2.687-19.576 8.39-44.304-.486-28.928-4.782M34.36 66.387s-4.286 3.173 2.26 3.85c8.464.873 15.15.945 26.717-1.283 0 0 1.6 1.622 4.115 2.51-23.668 6.92-50.03.545-33.092-5.077" />
        <path fill="#e76f00" d="M54.527 47.815c4.823 5.554-1.267 10.551-1.267 10.551s12.247-6.322 6.622-14.24C54.63 36.743 50.6 33.074 72.41 20.425c0 0-34.234 8.55-17.883 27.39" />
        <path fill="#5382a1" d="M80.418 85.434s2.827 2.33-3.115 4.133c-11.3 3.423-47.03 4.456-56.956.136-3.568-1.552 3.124-3.706 5.228-4.159 2.195-.475 3.45-.387 3.45-.387-3.968-2.795-25.648 5.49-11.012 7.861 39.913 6.473 72.757-2.914 62.405-7.584M38.59 55.044s-18.174 4.317-6.436 5.884c4.956.664 14.837.514 24.04-.258 7.522-.634 15.075-1.983 15.075-1.983s-2.653 1.136-4.571 2.446c-18.457 4.854-54.11 2.596-43.846-2.37 8.68-4.195 15.738-3.72 15.738-3.72m32.603 18.224c18.762-9.75 10.088-19.118 4.033-17.856-1.484.309-2.146.576-2.146.576s.551-.863 1.603-1.236c11.978-4.212 21.19 12.42-3.867 19.007 0 0 .29-.26.377-.491" />
        <path fill="#e76f00" d="M59.882 0s10.39 10.395-9.855 26.377C33.793 39.2 46.325 46.51 50.02 54.861c-9.476-8.55-16.43-16.077-11.765-23.082C45.103 21.496 64.075 16.51 59.882 0" />
        <path fill="#5382a1" d="M40.434 99.686c18.009 1.153 45.663-.64 46.318-9.161 0 0-1.26 3.23-14.883 5.796-15.371 2.892-34.329 2.555-45.573.7 0 0 2.302 1.906 14.138 2.665" />
      </svg>
    </svg>
  ),
  React: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="React">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <g clipPath="url(#r-a)">
          <path fill="#61dafb" d="M50.307 58.816a8.816 8.816 0 1 0 0-17.632 8.816 8.816 0 0 0 0 17.632" />
          <path stroke="#61dafb" strokeWidth="5" d="M50.307 68.063c26.126 0 47.306-8.087 47.306-18.063s-21.18-18.062-47.306-18.062C24.18 31.938 3 40.024 3 50s21.18 18.063 47.307 18.063Z" />
          <path stroke="#61dafb" strokeWidth="5" d="M34.664 59.031C47.727 81.658 65.321 95.957 73.96 90.97c8.64-4.988 5.053-27.374-8.01-50C52.885 18.342 35.291 4.043 26.652 9.03s-5.052 27.374 8.011 50Z" />
          <path stroke="#61dafb" strokeWidth="5" d="M34.664 40.969c-13.063 22.626-16.65 45.012-8.01 50 8.638 4.988 26.232-9.311 39.295-31.938s16.65-45.012 8.01-50c-8.638-4.988-26.232 9.311-39.295 31.938Z" />
        </g>
        <defs><clipPath id="r-a"><path fill="#fff" d="M0 0h100v100H0z" /></clipPath></defs>
      </svg>
    </svg>
  ),
  Vite: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Vite">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <g clipPath="url(#v-a)">
          <path fill="url(#v-b)" d="M98.848 14.801 52.731 98.695a2.486 2.486 0 0 1-4.365.019L1.336 14.808c-1.054-1.878.525-4.153 2.617-3.773L50.12 19.43c.294.053.596.053.89-.002l45.2-8.381c2.086-.387 3.672 1.871 2.638 3.753" />
          <path fill="url(#v-c)" d="M72.111.025 37.984 6.828c-.273.054-.52.2-.703.412a1.3 1.3 0 0 0-.307.764l-2.1 36.07a1.3 1.3 0 0 0 .442 1.05 1.25 1.25 0 0 0 1.091.268l9.502-2.23c.889-.21 1.692.587 1.51 1.496l-2.824 14.064c-.19.946.684 1.755 1.592 1.474l5.87-1.813c.909-.281 1.783.53 1.59 1.477l-4.486 22.09c-.28 1.382 1.526 2.135 2.28.951l.503-.791 27.81-56.46c.465-.946-.338-2.024-1.358-1.823l-9.781 1.92c-.92.18-1.701-.69-1.442-1.605l6.384-22.513c.26-.917-.526-1.788-1.446-1.604" />
        </g>
        <defs>
          <linearGradient id="v-b" x1=".187" x2="51.038" y1="17.805" y2="85.688" gradientUnits="userSpaceOnUse">
            <stop stopColor="#41d1ff" /><stop offset="1" stopColor="#bd34fe" />
          </linearGradient>
          <linearGradient id="v-c" x1="54.398" x2="64.98" y1="1.872" y2="73.224" gradientUnits="userSpaceOnUse">
            <stop stopColor="#ffea83" /><stop offset=".083" stopColor="#ffdd35" /><stop offset="1" stopColor="#ffa800" />
          </linearGradient>
          <clipPath id="v-a"><path fill="#fff" d="M0 0h100v100H0z" /></clipPath>
        </defs>
      </svg>
    </svg>
  ),
  TypeScript: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="TypeScript">
      <rect width="128" height="128" rx="16" fill="#3178C6" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <g clipPath="url(#t-a)">
          <path fill="#017acb" d="M0 0h100v100H0z" />
          <path fill="#fff" d="M48.016 37.031h4.797v8.282h-12.97v36.843l-.343.094c-.469.125-6.64.125-7.969-.016l-1.062-.093V45.312H17.5v-8.28l4.11-.048c2.25-.03 8.03-.03 12.843 0 4.813.032 10.906.047 13.563.047m36.61 41.219c-1.907 2.016-3.954 3.14-7.36 4.063-1.485.406-1.735.421-5.078.406-3.344-.016-3.61-.016-5.235-.438-4.203-1.078-7.594-3.187-9.906-6.172-.656-.843-1.734-2.593-1.734-2.812 0-.063.156-.203.359-.297s.625-.36.969-.562c.343-.204.968-.579 1.39-.797.422-.22 1.64-.938 2.703-1.579 1.063-.64 2.032-1.156 2.141-1.156.11 0 .313.219.469.485.937 1.578 3.125 3.593 4.672 4.28.953.407 3.062.86 4.078.86.937 0 2.656-.406 3.578-.828.984-.453 1.484-.906 2.078-1.812.406-.641.453-.813.438-2.032 0-1.125-.063-1.437-.375-1.953-.875-1.437-2.063-2.187-6.875-4.312-4.97-2.203-7.204-3.516-9.016-5.282-1.344-1.312-1.61-1.67-2.453-3.312-1.094-2.11-1.235-2.797-1.25-5.937-.016-2.204.031-2.922.265-3.672.329-1.125 1.391-3.297 1.875-3.844 1-1.172 1.36-1.531 2.063-2.11 2.125-1.75 5.438-2.906 8.61-3.015.359 0 1.546.062 2.656.14 3.187.266 5.359 1.047 7.453 2.72 1.578 1.25 3.968 4.187 3.734 4.577-.156.235-6.39 4.391-6.797 4.516-.25.078-.422-.016-.765-.422-2.125-2.547-2.985-3.094-5.047-3.219-1.469-.093-2.25.078-3.235.735-1.03.687-1.53 1.734-1.53 3.187.015 2.125.827 3.125 3.827 4.61 1.938.953 3.594 1.734 3.719 1.734.188 0 4.203 2 5.25 2.625 4.875 2.86 6.86 5.797 7.375 10.86.375 3.812-.703 7.296-3.047 9.765" />
        </g>
        <defs><clipPath id="t-a"><path fill="#fff" d="M0 0h100v100H0z" /></clipPath></defs>
      </svg>
    </svg>
  ),
  "Tailwind CSS": ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Tailwind CSS">
      <rect width="128" height="128" rx="16" fill="#0F172A" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <g clipPath="url(#tw-a)">
          <path fill="#06b6d4" d="M50 20q-20 0-25 19.994 7.5-9.997 17.5-7.498c3.804.95 6.522 3.71 9.532 6.764 4.902 4.974 10.576 10.731 22.969 10.731q20 0 24.999-19.995-7.5 9.997-17.5 7.5c-3.803-.951-6.521-3.71-9.531-6.765C68.067 25.758 62.392 20 50 20M25 49.991q-20 0-25 19.995 7.5-9.998 17.5-7.498c3.803.952 6.522 3.71 9.532 6.763C31.933 74.225 37.608 79.984 50 79.984q20 0 25-19.995-7.5 9.997-17.5 7.498c-3.803-.95-6.522-3.71-9.532-6.763C43.066 55.75 37.393 49.991 25 49.991" />
        </g>
        <defs><clipPath id="tw-a"><path fill="#fff" d="M0 0h100v100H0z" /></clipPath></defs>
      </svg>
    </svg>
  ),
  ShadcnUI: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="ShadcnUI">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <g clipPath="url(#s-a)">
          <path fill="#fff" d="M92.515 49.037a4.349 4.349 0 0 1 6.15 6.15L55.186 98.664a4.349 4.349 0 0 1-6.15-6.15zM83.82 1.211a4.349 4.349 0 0 1 6.149 6.15L7.361 89.968a4.349 4.349 0 0 1-6.15-6.15z" />
        </g>
        <defs><clipPath id="s-a"><path fill="#fff" d="M0 0h100v100H0z" /></clipPath></defs>
      </svg>
    </svg>
  ),
  GSAP: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="GSAP">
      <rect width="128" height="128" rx="16" fill="#0E100F" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <path fill="#0ae448" d="M50.21 44.196a.56.56 0 0 0 .169-.383h.002c.07-2.76-.569-4.916-1.897-6.408-1.403-1.576-3.552-2.385-6.387-2.405-2.83 0-5.269.884-7.052 2.557-1.686 1.584-2.637 3.775-2.68 6.171-.072 4.132 2.262 6.76 4.865 9.302 2.015 2.074 2.385 3.099 2.365 4.268-.025 1.353-.669 2.13-1.768 2.13-.453 0-.817-.141-1.078-.418-.408-.428-.588-1.201-.522-2.236a.5.5 0 0 0-.108-.392.56.56 0 0 0-.43-.212h-5.95a.557.557 0 0 0-.559.54c-.14 2.87.529 5.093 1.987 6.608 1.436 1.491 3.596 2.247 6.423 2.247 5.537 0 9.175-3.459 9.27-8.81.058-3.309-1.15-5.812-4.162-8.638-2.288-2.22-3.092-3.511-3.067-4.933.023-1.274.789-2.004 1.977-2.004s1.769.817 1.769 2.495a.69.69 0 0 0 .691.678h5.753a.56.56 0 0 0 .388-.157" />
        <path fill="#0ae448" d="M32.259 49.29v-.014h-.003c.005-.277-.188-.53-.46-.567q-.037-.006-.074-.005H21.197v.013c-.272.032-.493.217-.547.46l-1.096 4.762a.43.43 0 0 0 .089.369.6.6 0 0 0 .446.206h1.604c.092 0 .213.013.257.07.042.055.039.178-.005.337-.312 1.145-1.053 2.399-1.974 3.124-.922.724-2.445 1.158-3.511.505-1.203-.74-1.407-2.521-1.457-3.789-.105-2.722.428-5.456 1.474-7.972.749-1.801 2.161-5.051 4.555-5.051 1.357 0 2.04 1.12 2.091 3.423a.43.43 0 0 0 .415.417h6.245a.56.56 0 0 0 .56-.546c-.005-3.33-.765-5.872-2.262-7.561-1.416-1.597-3.452-2.397-6.045-2.374-8.524 0-12.95 8.612-13.888 17.147-.52 4.546.338 8.32 2.415 10.624 1.57 1.744 3.423 2.628 6.17 2.628 3.057 0 5.346-.679 7.202-2.134 2.182-1.708 3.835-4.554 5.056-8.694a.22.22 0 0 1 .211-.16h1.324c.31 0 .579-.196.638-.465z" />
        <path fill="#0ae448" fillRule="evenodd" d="m68.209 64.558.037-28.459a.535.535 0 0 0-.539-.543h-8.902c-.3 0-.431.258-.517.428l-12.89 28.4v.005l-.005.008c-.143.349.128.723.506.723h6.223q.505.002.669-.314l1.235-2.97c.152-.396.18-.433.613-.433h5.946c.413 0 .422.008.415.413l-.133 2.76a.54.54 0 0 0 .33.503.5.5 0 0 0 .208.04h6.287a.53.53 0 0 0 .41-.185.47.47 0 0 0 .107-.376m-11.014-9.501q-.07 0-.14-.005a.15.15 0 0 1-.126-.09.15.15 0 0 1-.007-.098 3 3 0 0 1 .055-.165l4.458-11.032a3 3 0 0 1 .139-.319c.072-.148.16-.159.187-.048.024.091-.511 11.327-.511 11.327-.042.42-.061.438-.476.473l-3.575-.041h-.008zm18.97-19.501h4.726l.005-.003c6.971 0 10.451 3.16 10.341 9.395-.128 7.306-4.58 12.675-11.284 13.913-.953.175-1.922.244-2.89.239l-3.237-.015a.204.204 0 0 0-.203.198.19.19 0 0 0 .105.173l.394.205 1.019.532 1.024.533.318.165c.226.118.313.314.263.566l-.718 3.237c-.059.27-.276.426-.594.426h-5.907a.58.58 0 0 1-.445-.206.43.43 0 0 1-.09-.37l6.578-28.558c.065-.297.345-.43.595-.43m2.991 16.995h.126c3.103-.126 4.794-3.792 4.856-7.348.036-2.055-.714-3.259-2.059-3.3h-4.32a.2.2 0 0 0-.175.299.2.2 0 0 0 .076.072s2.733 1.441 2.92 1.545c.147.08.15.217.101.454 0 .006-.418 1.844-.859 3.782-.492 2.163-1.01 4.451-1.008 4.455.008.01.058.04.342.04" clipRule="evenodd" />
      </svg>
    </svg>
  ),
  Git: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Git">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <path d="M116 58.8L69.2 12a8 8 0 0 0-11.3 0L47.5 22.4l14.2 14.2a9.5 9.5 0 0 1 12 12.1l13.7 13.7a9.5 9.5 0 1 1-5.7 5.4L68.5 54.4v31.4a9.5 9.5 0 1 1-7.8-.4V53.6a9.5 9.5 0 0 1-5.1-12.4L41.5 27 12 56.5a8 8 0 0 0 0 11.3L58.8 114a8 8 0 0 0 11.3 0L116 67.9a8 8 0 0 0 0-11.1z" fill="#F05032" />
    </svg>
  ),
  Vercel: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Vercel">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <path d="M64 24L112 104H16L64 24z" fill="#F5F2EB" />
    </svg>
  ),
  Cursor: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Cursor">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <g clipPath="url(#c-a)">
          <path fill="url(#c-b)" d="m50 100.136 43.437-25-43.437-25-43.438 25z" />
          <path fill="url(#c-c)" d="M93.438 75.137v-50L50 .136v50z" />
          <path fill="url(#c-d)" d="m50 .137-43.438 25v50l43.438-25z" />
          <path fill="#c3c3c3" d="m93.438 25.137-43.437 75v-50z" />
          <path fill="#fff" d="M93.437 25.137 50 50.137l-43.438-25z" />
        </g>
        <defs>
          <linearGradient id="c-b" x1="50" x2="50" y1="50.136" y2="100.136" gradientUnits="userSpaceOnUse">
            <stop offset=".16" stopColor="#fff" stopOpacity=".39" /><stop offset=".658" stopColor="#fff" stopOpacity=".8" />
          </linearGradient>
          <linearGradient id="c-c" x1="93.438" x2="50.001" y1="25.291" y2="50.761" gradientUnits="userSpaceOnUse">
            <stop offset=".182" stopColor="#fff" stopOpacity=".31" /><stop offset=".715" stopColor="#fff" stopOpacity="0" />
          </linearGradient>
          <linearGradient id="c-d" x1="50" x2="6.562" y1=".137" y2="75.136" gradientUnits="userSpaceOnUse">
            <stop stopColor="#fff" stopOpacity=".6" /><stop offset=".667" stopColor="#fff" stopOpacity=".22" />
          </linearGradient>
          <clipPath id="c-a"><path fill="#fff" d="M0 0h100v100H0z" /></clipPath>
        </defs>
      </svg>
    </svg>
  ),
  Claude: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Claude by Anthropic">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <path fill="#d97757" d="m95.815 38.848 3.465 1.98v1.486l-.99 3.465-42.08 9.901-3.956-9.83z" />
        <path fill="#d97757" d="m80.076 9.73 4.845 1.016 1.286 1.585 1.226 3.799-.508 2.422-28.239 38.613-9.406-9.406 26.04-34.17z" />
        <path fill="#d97757" d="m55.716 3.7 2.97-1.98 2.475.99 2.475 3.465-6.78 40.755-4.605-3.131-1.98-5.446L53.736 7.66z" />
        <path fill="#d97757" d="M23.959 4.352 27.01.454 29 0l3.95.577 1.949 1.526L49.102 33.59l5.137 14.961-6.01 3.342L25.32 10.322z" />
        <path fill="#d97757" d="m9.18 25.979-.99-3.962 2.972-3.465 3.465.495h.99L36.41 34.393l6.435 4.95 8.911 6.931-4.95 8.416-4.455-3.465-2.97-2.97-28.714-20.298z" />
        <path fill="#d97757" d="M3.24 51.72 1 49.244v-2.202l2.24-.768 25.248 1.485 24.753 1.98-.804 4.93-47.216-2.454z" />
        <path fill="#d97757" d="M18.092 77.488h-4.95l-1.968-2.266v-2.71l8.404-5.94 34.166-21.75 3.458 5.907z" />
        <path fill="#d97757" d="m27.498 91.324-1.98.495-2.97-1.486.495-2.475 29.208-38.614 3.96 5.446L34.43 83.403z" />
        <path fill="#d97757" d="m52.25 96.274-1.485 1.98-2.97.99-2.475-1.98-1.486-2.97 7.426-40.1 4.456.496z" />
        <path fill="#d97757" d="M77.499 85.383v3.96l-.495 1.485-1.98.99-3.466-.46-23.796-35.416 9.439-7.193 7.92 14.357.744 5.197z" />
        <path fill="#d97757" d="m88.885 79.442.495 2.475-1.485 1.98-1.485-.495-8.416-5.94-12.871-11.387-9.901-6.93 2.97-9.406 4.95 2.97 2.97 5.445z" />
        <path fill="#d97757" d="m82.448 54.195 12.377.99 2.97 1.98 1.98 2.97v2.138L94.33 64.59l-27.723-6.93-11.386-.496 2.97-10.396 7.921 5.94z" />
      </svg>
    </svg>
  ),
  Postman: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Postman">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 100 100">
        <g clipPath="url(#p-a)">
          <path fill="#ff6c37" d="M89.573 19.44a50 50 0 1 0-9.002 70.124c21.844-16.881 25.875-48.273 9-70.125z" />
          <path fill="#fff" d="M66.864 31.858 45.78 53.108l-5.958-6.125c20.75-20.75 22.833-18.875 27.042-15.125" />
          <path fill="#ff6c37" d="M45.78 53.689a.75.75 0 0 1-.5-.208l-6-6a.75.75 0 0 1 0-1.042c20.459-20.458 23.25-19.5 28.084-15.125a.67.67 0 0 1 .25.542.59.59 0 0 1-.208.543L46.323 53.483a.8.8 0 0 1-.542.208zm-4.916-6.708 4.917 4.917 20-20a4.5 4.5 0 0 0-5.918-1c-3.624 1.626-9.457 6.583-19 16.083" />
          <path fill="#fff" d="m51.823 58.983-5.75-5.875 21.083-20.834c5.667 5.542-2.75 14.792-15.333 26.707z" />
          <path fill="#ff6c37" d="M51.823 59.733a.9.9 0 0 1-.5-.208l-5.875-5.75a.88.88 0 0 1 0-.667.8.8 0 0 1 0-.542l21.083-21.083a.79.79 0 0 1 1.083 0 6.46 6.46 0 0 1 2.082 4.958c0 5.625-6.417 12.707-17.332 23.042a.88.88 0 0 1-.542.25zm-4.667-6.625 4.708 4.706c8.333-7.875 16.292-16 16.332-21.5a4.75 4.75 0 0 0-1.082-3.208z" />
          <path fill="#fff" d="m39.989 47.108 4.167 4.167a.25.25 0 0 1 0 .375l-8.792 1.875a.833.833 0 0 1-.75-1.417l5.125-5.083s.167 0 .25.083" />
          <path fill="#ff6c37" d="M35.156 54.358a1.62 1.62 0 0 1-1.375-.792 1.585 1.585 0 0 1 .292-1.875l5.083-5.125a1 1 0 0 1 1.333 0l4.167 4.167a.92.92 0 0 1 .25.917.97.97 0 0 1-.708.667l-8.792 1.875zm4.667-6.333-4.708 4.708 7.792-1.667z" />
          <path fill="#fff" d="M78.073 20.149a8.087 8.087 0 0 0-10.916-.286 8.08 8.08 0 0 0-1.24 10.848A8.086 8.086 0 0 0 76.615 32.9l-5.667-5.667z" />
          <path fill="#ff6c37" d="M72.448 34.814a8.83 8.83 0 0 1-8.056-5.481 8.86 8.86 0 0 1 1.875-9.56 8.92 8.92 0 0 1 12.5 0 .71.71 0 0 1 .184.841.7.7 0 0 1-.184.244l-6.75 6.375 5.125 5.042a.7.7 0 0 1 .208.625.75.75 0 0 1-.333.542 9.06 9.06 0 0 1-4.583 1.375zm0-16.167a7.3 7.3 0 0 0-6.74 4.531 7.36 7.36 0 0 0 1.574 7.97 7.29 7.29 0 0 0 8.083 1.541l-4.917-4.917a.667.667 0 0 1 0-1.042l6.5-6.542a7.48 7.48 0 0 0-4.5-1.542z" />
          <path fill="#fff" d="m78.198 20.274-7.125 7.083 5.667 5.667a8.6 8.6 0 0 0 1.583-1.167 8.333 8.333 0 0 0-.125-11.583" />
          <path fill="#ff6c37" d="M76.614 33.649a.63.63 0 0 1-.5-.208l-5.668-5.667a.667.667 0 0 1 0-1.042l7.083-6.958a.75.75 0 0 1 1.25 0 8.75 8.75 0 0 1 0 12.5c-.49.52-1.052.969-1.667 1.333a.67.67 0 0 1-.5.042zm-4.583-6.416 4.707 4.708q.486-.341.918-.75a7.38 7.38 0 0 0 2.167-5.208 7.16 7.16 0 0 0-1.792-4.75z" />
          <path fill="#fff" d="M67.739 32.733a3.043 3.043 0 0 0-4.168 0L44.698 51.564l3.125 3.167 19.958-17.5a3.09 3.09 0 0 0-.042-4.5z" />
          <path fill="#ff6c37" d="M47.656 55.483a.75.75 0 0 1-.5-.208l-3.167-3.167a.75.75 0 0 1 0-1.042l18.875-18.792a3.834 3.834 0 0 1 6.542 2.833 3.92 3.92 0 0 1-1.292 2.75l-19.958 17.5a.7.7 0 0 1-.5.124zm-2.208-3.919 2.083 2.125 19.583-17.042a2.3 2.3 0 0 0 .792-1.667 2.24 2.24 0 0 0-.708-1.708 2.333 2.333 0 0 0-3.292 0z" />
          <path fill="#fff" d="M30.073 73.024a.42.42 0 0 0 0 .5l.833 3.624a1.07 1.07 0 0 1-.519 1.094 1.02 1.02 0 0 1-1.19-.12l-5.708-5.5 17.792-17.933h6.167l4.167 4.167a94.6 94.6 0 0 1-21.542 14.167" />
          <path fill="#ff6c37" d="M29.698 79.148a1.76 1.76 0 0 1-1.25-.542l-5.457-5.457a.667.667 0 0 1 0-1.043L40.906 54.15c.1-.107.231-.18.375-.208h6.208a.75.75 0 0 1 .5 0l4.167 4.167a.67.67 0 0 1 .208.583.67.67 0 0 1-.25.542l-.333.292a102 102 0 0 1-21.375 13.708l.792 3.375a1.798 1.798 0 0 1-1.707 2.208zm-5.082-6.542 4.915 4.958a.25.25 0 0 0 .333 0 .25.25 0 0 0 0-.292l-.833-3.625a1.16 1.16 0 0 1 .625-1.333A102.4 102.4 0 0 0 50.49 58.897l-3.417-3.417H41.49z" />
          <path fill="#fff" d="m19.198 76.898 4.167-4.167 6.418 6.417-10.208-.707A.834.834 0 0 1 19.2 76.9z" />
          <path fill="#ff6c37" d="m29.906 79.773-10.167-.708a1.61 1.61 0 0 1-1.334-1.02 1.59 1.59 0 0 1 .334-1.647l4.167-4.167a.75.75 0 0 1 1.042 0l6.375 6.375a.71.71 0 0 1 0 .833.8.8 0 0 1-.417.333zm-10.167-2.332 8.333.542-4.583-4.293z" />
          <path fill="#fff" d="m48.03 55.192-8.416 1.293 4.875-4.833z" />
          <path fill="#ff6c37" d="M39.614 57.274a.71.71 0 0 1-.667-.418.7.7 0 0 1 0-.832l4.832-4.876a.79.79 0 0 1 1.083 0l3.544 3.541a.7.7 0 0 1 0 .75.74.74 0 0 1-.583.542l-8.333 1.293zm4.875-4.543L41.78 55.44l4.667-.708z" />
          <path fill="#fff" d="m48.03 55.19-5.874.75a.75.75 0 0 1-.708-1.25l3.042-3.043z" />
          <path fill="#ff6c37" d="M41.989 56.733a1.54 1.54 0 0 1-1.333-.792 1.5 1.5 0 0 1 .25-1.793l3.042-3.042a.79.79 0 0 1 1.083 0l3.542 3.542a.75.75 0 0 1-.458 1.293l-5.833.75zm0-1.542 4.417-.543-1.958-1.957-2.5 2.5zm36.25-30.125a.625.625 0 0 0-.793-.25.58.58 0 0 0-.29.344.6.6 0 0 0 .04.448c.292.614.211 1.34-.208 1.875a.53.53 0 0 0-.156.395.6.6 0 0 0 .156.396.45.45 0 0 0 .333 0 .55.55 0 0 0 .542-.168 2.88 2.88 0 0 0 .375-3.042z" />
        </g>
        <defs><clipPath id="p-a"><path fill="#fff" d="M0 0h100v100H0z" /></clipPath></defs>
      </svg>
    </svg>
  ),
  Gemini: ({ size }) => (
    <svg width={size} height={size} viewBox="0 0 128 128" fill="none" aria-label="Gemini">
      <rect width="128" height="128" rx="16" fill="#1a1a1a" />
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none">
        <path d="M50 8c.7 11.2 3.7 19.4 8.6 25.7 4.5 5.7 10.5 10.1 17.7 13.1-7.2 3-13.2 7.4-17.7 13.1-4.9 6.3-7.9 14.5-8.6 25.7-.7-11.2-3.7-19.4-8.6-25.7-4.5-5.7-10.5-10.1-17.7-13.1 7.2-3 13.2-7.4 17.7-13.1C46.3 27.4 49.3 19.2 50 8z" fill="url(#g-grad)" />
        <defs>
          <linearGradient id="g-grad" x1="0" y1="0" x2="100" y2="100">
            <stop stopColor="#4285F4" /><stop offset=".5" stopColor="#34A853" /><stop offset="1" stopColor="#FBBC04" />
          </linearGradient>
        </defs>
      </svg>
    </svg>
  ),
};

function TechLogo({ name, size = 52 }: { name: string; size?: number }) {
  const Logo = logos[name];
  if (Logo) return <Logo size={size} />;
  const initials = name
    .split(" ")
    .map((w) => w[0])
    .join("")
    .slice(0, 2)
    .toUpperCase();
  return (
    <svg width={size} height={size} viewBox="0 0 52 52" fill="none" aria-label={name}>
      <rect width="52" height="52" rx="8" fill="#2A2A2A" />
      <text x="26" y="34" textAnchor="middle" fill="#F5F2EB" fontSize="16" fontFamily="Georgia, serif" fontWeight="300">
        {initials}
      </text>
    </svg>
  );
}

const stack = [
  {
    category: "Backend",
    kanji: "バック",
    items: ["Django", "Python", "Java"],
  },
  {
    category: "Frontend",
    kanji: "フロント",
    items: ["React", "Vite", "TypeScript", "Tailwind CSS", "ShadcnUI", "GSAP"],
  },
  {
    category: "Tools",
    kanji: "ツール",
    items: ["Git", "Vercel", "Cursor", "Claude", "Postman", "Gemini"],
  },
];

function TechCard({ name, index }: { name: string; index: number }) {
  const [hovered, setHovered] = useState(false);
  const shouldReduce = useReducedMotion();

  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, margin: "-40px" }}
      transition={{
        duration: 0.35,
        delay: index * 0.045,
        ease: [0.23, 1, 0.32, 1],
      }}
    >
      <motion.div
        className="group relative flex flex-col items-center gap-3 p-4 border border-hairline bg-surface/30 rounded-lg cursor-default select-none"
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        animate={
          shouldReduce
            ? {}
            : {
                borderColor: hovered ? "#DC2626" : "#2A2A2A",
                backgroundColor: hovered
                  ? "rgba(220,38,38,0.04)"
                  : "rgba(22,22,22,0.3)",
              }
        }
        transition={{ duration: 0.18, ease: "easeOut" }}
      >
        <motion.div
          animate={shouldReduce ? {} : { scale: hovered ? 1.07 : 1 }}
          transition={{ duration: 0.2, ease: [0.23, 1, 0.32, 1] }}
          className="flex items-center justify-center"
        >
          <TechLogo name={name} size={52} />
        </motion.div>

        <span className="text-label font-mono uppercase tracking-widest text-ash group-hover:text-bone transition-colors duration-150 text-center leading-tight">
          {name}
        </span>

        <motion.div
          className="absolute bottom-0 left-0 h-[1.5px] bg-vermilion origin-left rounded-full"
          animate={shouldReduce ? {} : { scaleX: hovered ? 1 : 0 }}
          transition={{ duration: 0.2, ease: [0.23, 1, 0.32, 1] }}
          style={{ width: "100%" }}
        />
      </motion.div>
    </motion.div>
  );
}

export function TechStack() {
  return (
    <section
      id="stack"
      className="py-section px-gutter bg-surface border-y border-hairline"
    >
      <div className="max-w-content mx-auto">
        <FadeIn>
          <div className="flex items-end gap-4 mb-20 border-b border-hairline pb-6">
            <span className="text-vermilion text-sm font-medium tracking-widest">
              技術
            </span>
            <span className="text-caption text-ash font-mono uppercase tracking-widest">
              / 03 — Tech Stack
            </span>
          </div>
        </FadeIn>

        <div className="flex flex-col gap-16">
          {stack.map((group, gi) => (
            <FadeIn key={gi} delay={gi * 0.08}>
              <div>
                <div className="flex items-baseline gap-3 mb-8">
                  <h3 className="font-serif italic text-2xl text-bone">
                    {group.category}
                  </h3>
                  <span className="text-label font-mono text-vermilion tracking-widest opacity-60">
                    {group.kanji}
                  </span>
                  <div className="flex-1 h-[1px] bg-hairline ml-2" />
                </div>

                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                  {group.items.map((item, ii) => (
                    <TechCard key={item} name={item} index={gi * 6 + ii} />
                  ))}
                </div>
              </div>
            </FadeIn>
          ))}
        </div>
      </div>
    </section>
  );
}
